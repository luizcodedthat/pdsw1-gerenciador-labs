...
<template>
  <div class="chamado-card">
    ...
    <button
      v-if="chamado.status === 'Aberto' || chamado.status === 'Em andamento'"
      class="btn-atender"
      @click="abrir"
    >
      Atender chamado
...
.chamado-card:hover { box-shadow: 0 2px 8px rgba(0,0,0,0.15); }
...
.status.aberto { background: #fde68a; color: #92400e; }
.status.em-andamento { background: #fdba74; color: #9a3412; }
.status.concluído { background: #bbf7d0; color: #166534; }
.status.fechado { background: #fecaca; color: #991b1b; }
